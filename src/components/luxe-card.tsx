...
/**
 * Ivory plaque with a gold hairline, layered drop-shadows and a
 * live 3D cursor-tilt. Inner elements can lift on the Z-axis using
 * `style={{ transform: 'translateZ(Npx)' }}` on the children. Used
 * on dark ink panels where the cream body creates maximum contrast.
 */
export function LuxeCard({ index, eyebrow, title, children }: LuxeCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Gold rule */}
        <div className="gold-divider my-6" style={{ transform: "translateZ(20px)" }} />

        {/* Content */}
        <div
          className="relative text-[var(--espresso-700)] leading-relaxed"
          style={{ transform: "translateZ(30px)" }}
        >
          {children}
        </div>
